...
/// Characters that indicate quotes in oder of likelihood.
static QUOTES: &'static [u8; 2] = &[b'"', b'\''];
...
impl<'a> SAXParser<'a> {
    ...
    fn sgml_decl(&mut self, gc: &mut GraphemeClusters, current: &[u8]) {
        ...
        let bytes = current;
        if is_sgml_char {
            self.sgml_decl.value.extend_from_slice(bytes);
        } else {
            self.sgml_decl = Text::new([0, 0]);
        }

        if match_byte(QUOTES, bytes[0]) {
    ...
    fn sgml_quoted(&mut self, current: &[u8]) {
        let bytes = current;
        let maybe_quote = unsafe { *bytes.get_unchecked(0) };
    ...
    fn doctype(&mut self, gc: &mut GraphemeClusters, current: &[u8]) {
        ...
        let bytes = current;
        self.doctype.value.extend_from_slice(bytes);
        if current[0] == b']' {
            self.state = State::DoctypeDtd;
        } else if match_byte(QUOTES, bytes[0]) {
            self.state = State::DoctypeQuoted;
            self.quote = unsafe { *bytes.get_unchecked(0) };
        }
    }
    ...
    fn proc_inst(&mut self, gc: &mut GraphemeClusters, current: &[u8]) {
        ...
        let bytes = current;
        if match_byte(WHITESPACE, bytes[0]) {
            self.proc_inst.target.end = [gc.line, gc.character - 1];
            self.state = State::ProcInstValue;
            return;
        }

        self.proc_inst.target.value.extend_from_slice(bytes);
    ...
    fn attribute_value(&mut self, gc: &mut GraphemeClusters, current: &[u8]) {
      ...
      let first_byte = unsafe { *bytes.get_unchecked(0) };
        ...
        if match_byte(QUOTES, first_byte) {
            ...
            self.attribute.value.value.extend_from_slice(bytes);
    ...
    fn process_close_tag(&mut self, gc: &mut GraphemeClusters) {
        ...
        let close_tag_name =
            if self.tag.self_closing && self.close_tag_name.is_empty() { &self.tag.name } else { &mem::take(&mut self.close_tag_name) };
